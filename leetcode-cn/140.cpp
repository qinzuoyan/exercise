...
public:
    ...
    bool wordBreak(vector<string>& r, string& t,
            const string& s, size_t startpos, vector<int>& vec, TrieNode* trie) {
        if (startpos >= s.size()) {
            r.push_back(t);
            return true;
        }
    ...
    vector<string> wordBreak(string s, vector<string>& wordDict) {
        ...
        vector<string> r;
        string t;
        vector<int> vec(s.size()); // -1 means false, 1 means true, 0 means not processed.
