...
class Solution {
 public:
  void update_self(vector<vector<int>>& a, int n, int m, int i, int j,
    ...
    a[i][j] = c;
    if (c == 1) s1.emplace(i, j);
    else if (c == 2) s2.emplace(i, j);
  }
  ...
  void update_neighbor(vector<vector<int>>& a, int n, int m, int i, int j,
    ...
    if (i > 0 && a[i - 1][j] != -1) {
      --a[i - 1][j];
      if (a[i - 1][j] == 1) s1.emplace(i - 1, j);
      else if (a[i - 1][j] == 2) s2.emplace(i - 1, j);
    }
    if (i < n - 1 && a[i + 1][j] != -1) {
      --a[i + 1][j];
      if (a[i + 1][j] == 1) s1.emplace(i + 1, j);
    ...
    if (j > 0 && a[i][j - 1] != -1) {
      --a[i][j - 1];
      if (a[i][j - 1] == 1) s1.emplace(i, j - 1);
      else if (a[i][j - 1] == 2) s2.emplace(i, j - 1);
    }
    if (j < m - 1 && a[i][j + 1] != -1) {
      --a[i][j + 1];
      if (a[i][j + 1] == 1) s1.emplace(i, j + 1);
