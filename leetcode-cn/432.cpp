...
DListNode* insert_before(DListNode* head, DListNode* insert, DListNode* before) {
    if (head == nullptr) {
...
public:
    ...
    void inc(string key) {
        ...
        if (it == map_.end()) {
            ...
            cnodeNext->key_node = insert_before(cnodeNext->key_node, knode, cnodeNext->key_node);
        }
    }
    ...
    void dec(string key) {
        ...
        if (it != map_.end()) {
            ...
            if (cnodePrev != nullptr) {
                ...
                cnodePrev->key_node = insert_before(cnodePrev->key_node, knode, cnodePrev->key_node);
            }
            else {
                delete knode;
                map_.erase(it);
