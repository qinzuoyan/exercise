...
private:
    struct Node {
        ...
        int left, right;
        Node* lptr, *rptr;
        Node(int l, int r): sum(0), left(l), right(r), lptr(nullptr), rptr(nullptr) {}
    };
    ...
    int sumRange(Node* node, int left, int right) {
        ...
        if (left <= node->left && right >= node->right)
            return node->sum;
        return sumRange(node->lptr, left, right) + sumRange(node->rptr, left, right);
    }

public:
    NumArray(vector<int>& nums) {
        _root = construct(nums, 0, nums.size() - 1);
    }
    
    void update(int index, int val) {
        update(_root, index, val);
    }
